...
export interface ChannelColorDialogProps {
    ...
    adapterName?: string;
    instance?: number;
    onSave?: (config: ChannelColorConfig) => void;
...
interface ChannelColorDialogState {
    open: boolean;
 ...
 */
class ChannelColorDialog extends React.Component<ChannelColorDialogProps, ChannelColorDialogState> {
    constructor(props: ChannelColorDialogProps) {
        super(props);
        this.state = {
            open: false,
            colorBest: '',
    ...
    override componentDidUpdate(prevProps: ChannelColorDialogProps): void {
        ...
        if (!prevProps.trueColor && this.props.trueColor) {
            void this.loadAdapterColorTheme();
        }
    }
    ...
    private handleCancel = (): void => {
        // Lokale Änderungen verwerfen und Original-Werte wiederherstellen
        this.setState({
            open: false,
            localTrueColor: this.props.trueColor,
            localFalseColor: this.props.falseColor,
     ...
     */
    public openWith(config?: Partial<ChannelColorConfig>): void {
        if (config?.scale) {
    ...
    render(): React.JSX.Element {
        const { open, colorBest, colorMode, colorLog10, valMin, valMax, valBest /*, valIconMin, valIconMax*/ } =
            this.state;
        ...
        return (
            ...
            >
                ...
                <DialogActions>
                    <Button onClick={this.handleCancel}>{I18n.t('channelConfigColor_cancel')}</Button>
                    <Button
                        onClick={this.handleSave}
                        variant="contained"
                        color="primary"
                    >
                        {I18n.t('channelConfigColor_save')}
                    </Button>
                </DialogActions>
            </Dialog>
        );
...
export default ChannelColorDialog;
