...
class Panel extends import_library.BaseClass {
  ...
  onMessage = async (topic, message) => {
    ...
    if (topic.endsWith(definition.ReiveTopicAppendix) || topic.endsWith("/stat/RESULT")) {
      ...
      if (event) {
        ...
        if ("Flashing" in msg) {
          ...
          if (((_a = this.info.tasmota.sensors) == null ? void 0 : _a.TempUnit) === "F") {
            def.common.unit = "\xB0F";
          } else {
            def.common.unit = "\xB0C";
          }
      ...
      this.info.tasmota.sts = JSON.parse(message);
  ...
  sendRules() {
    this.sendToTasmota(
      ...
      `ON CustomSend DO RuleTimer3 140 ENDON ON Rules#Timer=3 DO CustomSend pageType~pageStartup ENDON${this.detach.left ? ` ON Button1#state do Publish ${this.topic}/tele/RESULT {"CustomRecv":"event,button1"} ENDON` : ""}${this.detach.right ? ` ON Button2#state do Publish ${this.topic}/tele/RESULT {"CustomRecv":"event,button2"} ENDON` : ""}`
    );
