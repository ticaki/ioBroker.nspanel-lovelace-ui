...
var Definition = __toESM(require("./definition"));
var Color = __toESM(require("./color"));
var Icons = __toESM(require("./icon_mapping"));
var import_moment = __toESM(require("moment"));
var import_moment_parseformat = __toESM(require("moment-parseformat"));
var import_msg_def = require("./msg-def");
...
class Screensaver extends import_panel_message.BaseClassPanelSend {
  ...
  async init() {
    this.timeUpdateTimeout = this.adapter.setTimeout(this.timeUpdateLoop, 1e3);
  ...
  async delete() {
    ...
    if (this.timeUpdateTimeout)
      this.adapter.clearTimeout(this.timeUpdateTimeout);
  }
  async update() {
  ...
  async HandleScreensaverStatusIcons() {
    ...
    for (let i = 0; i < maxItems; i++) {
      ...
      const offcolor = item.entityOffColor ? await item.entityOffColor.getRGBDec() : null;
      const onColor = item.entityOnColor ? await item.entityOnColor.getRGBDec() : null;
      payload[`icon${s}Color`] = offcolor !== null ? offcolor : String(Color.rgb_dec565(Color.Black));
