...
Anstatt Farben als JSON-Objekte zu definieren, können Sie die im Skript verfügbaren Farbkonstanten verwenden. Dies macht die Configuration übersichtlicher und weniger fehleranfällig:
...
Das `iconScaleElement` ermöglicht eine erweiterte Configuration der Icon-Farbgebung mit verschiedenen Modi und Optionen:
...
### Wichtiger Hinweis
**Alle Wetter-Templates funktionieren nur mit korrekter `dpInit`-Configuration!** Der `dpInit`-Parameter muss auf den entsprechenden Wetteradapter und dessen State-Struktur angepasst werden.
...
## Advanced Mode - Komplett-Configuration
...
const config: ScriptConfig.Config = {
    ...
    // KEINE mrIcon-Configuration - Easy View unterstützt keine MR-Icons
...
Hier ist eine vollständige Screensaver-Configuration mit allen Bereichen:

```typescript
const config: ScriptConfig.Config = {
    // ...andere Basis-Configuration
...
3. **Versionierung**: Dokumentieren Sie Änderungen an der Screensaver-Configuration
...
- Kontrollieren Sie die Syntax der Configuration
