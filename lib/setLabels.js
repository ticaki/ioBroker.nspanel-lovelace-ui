...
async function doIt() {
    ...
    const pr = await getGithub(`https://api.github.com/repos/ticaki/ioBroker.nspanel-lovelace-ui/pulls/${prID}`);
    const sha = pr.head.sha;

    const actions = await getGithub(
        `https://api.github.com/repos/ticaki/ioBroker.nspanel-lovelace-ui/actions/workflows/test-and-release.yml/runs`,
        false,
        { head_sha: sha },
    );
